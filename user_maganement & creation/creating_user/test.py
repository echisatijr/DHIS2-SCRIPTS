...
def find_created_users_file():
    candidates = [f"data/{district}_created_users.xlsx"]
...
def send_users_to_dhis2(df_users, district_name, output_file=f"data/{district}_created_users.xlsx"):
    created_users = []
    for _, user in df_users.iterrows():
        ...
        if response.status_code == 201:
            ...
            created_users.append({
                ...
                "Facility Name": user["parent_orgunit_name"],
                "Username": user["username"],
                "Password": user["password"],
                "UID": created_uid,
